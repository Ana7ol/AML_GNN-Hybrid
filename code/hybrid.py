...
def create_transaction_sequences(df_original_with_ts, features_processed, sequence_length=10, step_size=5):
    ...
    for _, group in df_temp.groupby('Account'):
        ...
        if num_transactions < sequence_length:
            ...
            all_sequences.append(padded_features)
            all_labels.append(labels[-1])
        else:
            for i in range(0, num_transactions - sequence_length + 1, step_size):
                all_sequences.append(transactions[i : i + sequence_length])
                all_labels.append(labels[i + sequence_length - 1])
...
if __name__ == '__main__':
    ...
    X_sequences_np, _ = create_transaction_sequences(df_gnn, X_features_np, SEQUENCE_LENGTH, STEP_SIZE)
    ...
    # ADDED: Learning rate scheduler for stability
    ...
    individual_dataloader = DataLoader(individual_dataset, batch_size=BATCH_SIZE * 4, shuffle=False, num_workers=0, pin_memory=True)
    ...
    def train_gnn():
        ...
        h = gnn_model.encode(train_graph.edge_index)
        preds = gnn_model.decode(h, train_graph.edge_label_index, train_graph.edge_attr)
    ...
    def test_gnn(graph):
        gnn_model.eval()
        h = gnn_model.encode(graph.edge_index)
        preds = gnn_model.decode(h, graph.edge_label_index, graph.edge_attr)
